...
public class BooleanQueryParser {
	...
	private Literal findNextLiteral(String subquery, int startIndex) {
		...
		if (subquery.charAt(startIndex) == '[') {
			...
			int d = Character.getNumericValue(distance.charAt(distance.length() - 1));
			String secondTerm = termList.get(2);

			return new Literal(new StringBounds(startIndex, lengthOut), new NearLiteral(firstTerm, secondTerm, d));
		}
